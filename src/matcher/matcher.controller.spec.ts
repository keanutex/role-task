...
describe('MatcherController', () => {
  ...
  describe('MatchAll', () => {
    it('should return a match of all candidates', async () => {
      ...
      const candidate1ProductManager = matches.find(
        (match) =>
          match.roleName == 'Product Manager' && match.userName == 'Candidate1',
      );
      const candidate2ProductManager = matches.find(
        (match) =>
          match.roleName == 'Product Manager' && match.userName == 'Candidate2',
      );
      const candidate1CustomerSuccess = matches.find(
        (match) =>
          match.roleName == 'Head of Customer Success' &&
          match.userName == 'Candidate1',
      );

      expect(candidate1ProductManager!.score).toEqual(100);
      expect(candidate2ProductManager!.score).toEqual(77);
      expect(candidate1CustomerSuccess!.score).toEqual(null);
    });
  });
});
